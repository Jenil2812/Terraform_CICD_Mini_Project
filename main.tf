terraform {
  ...
  backend "azurerm" {
    resource_group_name = "Azure_Project_RG"
    storage_account_name = "azureinfraproject"
...
provider "azurerm" {
  features {}
  subscription_id = "bbc57f82-2406-4700-acc3-f42037787e41"
}
...
resource "azurerm_linux_virtual_machine" "mylinuxvm" {
  ...
  admin_ssh_key {
    username   = "azureadmin"
    public_key = file("~/.ssh/azure_keyb.pub") #Public Key to VM
  }
...
resource "null_resource" "file_copy_remote_exec" {
  provisioner "file" {
    ...
    connection {
      ...
      user        = "azureadmin"
      private_key = file("~/.ssh/azure_keyb")
    }
  }
